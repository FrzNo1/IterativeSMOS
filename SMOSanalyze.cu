...
*/
  ...
  if(printBool) {
    fileCsv << "k value count, " << kListCount << ", " << "2^" << (int) log2((float) size) << ", ratio:" << (100*((float)kListCount/size)) << "," << namesOfGeneratingFunctions[generateType] << "," << namesOfKGenerators[kGenerateType] << "," << seed << ",";
    for(x = 0; x < NUMBEROFALGORITHMS; x++)
...
int main (int argc, char *argv[]) {
  ...
  uint testCount, type,distributionType,startPower,stopPower,kDistribution;
  uint vecDistr[4];
  ...
  for(int j=0; j<4; j++){
    ...
    for(type=0; type<3; type++){
      ...
      case 0:
        typeString = "float";
          snprintf(fileName, 128, "CR %s %s k-dist:%s 2^%d to 2^%d %d-tests on %s at %s", 
               typeString, getDistributionOptions(type, distributionType), getKDistributionOptions(kDistribution), 
               startPower, stopPower, testCount, hostName, humanTime);
        printf("File Name: %s \n", fileName);
