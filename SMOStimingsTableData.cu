...
//  uint i;
  ...
  */
  ...
  for(size = (1 << startPower); size <= (1 << stopPower); size *= 4) {
...
int main (int argc, char *argv[]) {
  ...
  numKs[0]=5;     // standard descriptive statistics
  numKs[1]=11;    // ten percentiles
  numKs[2]=101;   // one percentiles
  numKs[3]=1001;  // 1/10th percentiles
 ...
 for (int i=0; i<4; i++){
  ...
  startK=numKs[i];       // This gives the 1/(startK-1) percentiles including minimum and maximum
  jumpK=1;
  stopK=startK;
